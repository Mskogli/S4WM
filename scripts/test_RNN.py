...
def main(cfg: DictConfig) -> None:
    ...
    params = model.restore_checkpoint_state(
        "/home/mathias/dev/structured-state-space-wm/scripts/checkpoints/depth_dataset/d_model=512-lr=0.0001-bsz=2-latent_type=cont/checkpoint_28"
    )["params"]
    ...
    pred_depth = out["depth"]["pred"]
    recon_depth = out["depth"]["recon"]
    for i in range(10):
        plt.imsave(f"imgs/pred_rnn_{i}.png", pred_depth[0, i + 60, :].reshape(270, 480))
        plt.imsave(f"imgs/recon_{i}.png", recon_depth[0, i + 60, :].reshape(270, 480))
