...
def test_dreaming(model_conf: DictConfig) -> None:
    ...
    model = BatchStackedModel(layer_cls=S4Layer,  d_output=128, classification=False, training=False, decode=True, **model_conf)
    ...
    for i in range(L_DREAM):
        ...
        pred_latent_tensor = torch.from_numpy(np.asarray(preds[13, :, :])).to(torch.device("cuda:0"))
